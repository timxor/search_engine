...
public class BaseProjectTest {
    ...
    public static final String QUERY_OUT = "-results";

    public static final String SERVER_PORT = "-port";
    public static final String THREAD_NUM = "-multi";
    ...
    public static final Path OUTPUT_DIR = Paths.get("out");
    ...
    public static boolean isEnvironmentSetup() {
        ...
        return Files.isReadable(INPUT_DIR) && Files.isReadable(EXPECTED_DIR)
                && Files.isReadable(OUTPUT_DIR) && Files.isWritable(OUTPUT_DIR);
    }
     ...
     */
    public static String errorMessage(String test, String[] args,
            String message) {
        return String.format(
                "%n" + "Test: %s%n" + "Args: %s%n" + "Message: %n%s%n", test,
    ...
    public static void checkExceptions(String name, String[] args) {
        try {
            System.out.printf("%nRunning %s...%n", name);
            Driver.main(args);
        }
        catch (Exception e) {
     ...
     * Checks whether {@link Driver} generates the expected output without any
     * exceptions. Will print the stack trace if an exception occurs. Designed
     * to be used within an unit test. If the test was successful, deletes the
     * actual file. Otherwise, keeps the file for debugging purposes.
     *
     * @param test
     ...
     *            arguments to pass to {@link Driver}
     */
    public static void checkProjectOutput(String test, Path expect, Path actual,
            String[] args) {
        ...
        try {
            ...
            System.out.printf("%nRunning: %s...%n", test);
            Driver.main(args);

            // Double-check we can read the expected output file
            Assert.assertTrue(
                    errorMessage(test, args,
            ...
            // At this stage, the files were the same. We can safely delete
            // actual.
            Files.deleteIfExists(actual);
        }
        catch (Exception e) {
